...
interface ApiNotificationConfig { // Estrutura retornada pela API GET
    id: number;
    ...
    frequency: string;
    startPeriod: number;
    isActive: boolean;  // <--- Usa isActive (camelCase padrão)
    createdAt?: string;
    updatedAt?: string;
}
...
interface FormDataState {
    ...
    frequency: string;
    startPeriod: number; // <-- Usa camelCase para consistência interna
}

// Create Payload (DTO para POST /create)
interface CreateNotificationRequestPayload {
    clientsIDs: number[];
    taxTypeIDs: number[];
    notificationTypeList: number[];
    frequency: string;
    active: boolean;  // <--- CORRIGIDO: Usa 'active' conforme pedido para ESTE DTO
    startPeriod: number;
}
...
const Notifications: React.FC = () => {
    ...
    const initialFormData: FormDataState = {
        ...
        startPeriod: 1, // <-- Usa camelCase
    };
    const [formData, setFormData] = useState<FormDataState>(initialFormData);
    const [isActiveEdit, setIsActiveEdit] = useState<boolean>(true); // <-- Usa isActiveEdit
    ...
    const groupedConfigs = useMemo<GroupedConfig[]>(() => {
        ...
        originalConfigs.forEach((config) => {
            ...
            const isActive = config.isActive; // <-- Usa isActive da API
            ...
            const groupKey = `${notificationTypeId}-${taxTypeId}-${freq}-${startP}-${isActive}`; // <-- Usa isActive na chave

            if (!groups[groupKey]) {
                groups[groupKey] = {
                    key: groupKey, notificationType: config.notificationType, taxType: config.taxType,
                    frequency: freq, startPeriod: startP, isActive: isActive, // <-- Usa isActive no grupo
                    clients: [], originalIds: []
                };
            }
    ...
    const filteredGroupedConfigs = useMemo<GroupedConfig[]>(() => {
        ...
        if (filterStatus !== 'all') {
            ...
            configs = configs.filter(group => group.isActive === isActiveFilter); // <-- Usa isActive
        }
        return configs;
    ...
    const getSubmitBody = useCallback((): CreateNotificationRequestPayload | any => {
        if (!editingConfigId) { // CRIAÇÃO
            const payload: CreateNotificationRequestPayload = {
                ...
                active: true, // <--- CORRIGIDO: Usa 'active' conforme DTO de Criação
                startPeriod: formData.startPeriod, // <-- Usa camelCase
            };
            ...
            const updatePayload: any = {
                ...
                frequency: formData.frequency,
                startPeriod: formData.startPeriod, // <-- Usa camelCase
                isActive: isActiveEdit, // <-- Usa isActive e estado correto para edição
    ...
    const { data: updateResponse, loading: updateLoading, error: updateError } = useApi<ApiResponseData>(submitEndpoint, { method: submitMethod, body: getSubmitBody(), enabled: submitTrigger > 0 && !!editingConfigId });
    ...
    const resetFormAndState = useCallback(() => { setFormData(initialFormData); setIsActiveEdit(true); setSelectedClients([]); setClientSelectionMode('individual'); setEditingConfigId(null); setClientSearchTerm(''); setShowClientDropdown(false); setSubmitTrigger(0); setActionDetails(null); setActionTrigger(0); setNotificationMessage(null); setCreateLoading(false); }, [initialFormData]); // CORRIGIDO: setIsActiveEdit
    useEffect(() => { if (editingConfigId && submitTrigger > 0) { if (!updateLoading) { if (updateError) { setNotificationMessage({ type: 'error', text: `Erro ao atualizar: ${updateError}` }); } else { setNotificationMessage({ type: 'success', text: `Configuração atualizada!` }); resetFormAndState(); setRefreshTrigger(prev => prev + 1); } setSubmitTrigger(0); } } }, [submitTrigger, updateLoading, updateError, editingConfigId, resetFormAndState, setRefreshTrigger]);
    ...
    const handleStandardChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        // CORRIGIDO: Nome do campo no estado é 'startPeriod'
    ...
    const handleEdit = useCallback((config: ApiNotificationConfig) => {
        ...
        setFormData({
            ...
            startPeriod: config.startPeriod, // <-- Usa camelCase
        });
        setIsActiveEdit(config.isActive); // <-- Usa isActive da API e setter correto
    ...
    const handleTableAction = useCallback((action: 'delete' | 'toggle', originalConfigId: number, currentStatus?: boolean) => {
         ...
         const deleteUrl = `${FULL_API_BASE_URL}/notification/delete/${originalConfigId}`;
         const updateUrl = `${FULL_API_BASE_URL}/notification/update/${originalConfigId}`;
         if (action === 'delete') {
             ...
             details = {
                 ...
                 method: 'PUT',
                 body: { isActive: !currentStatus } // <-- CORRIGIDO: Usa 'isActive'. VERIFICAR PAYLOAD **
             };
         }
          if (details) { setActionDetails(details); setActionTrigger(prev => prev + 1); }
     }, []); // Adicionar FULL_API_BASE_URL se não for constante pura
    ...
    if ((clientsError || taxTypesError || configsError) && originalConfigs.length === 0) { const err = clientsError || taxTypesError || configsError; return <div id="notif-msg-standalone-error"><FontAwesomeIcon icon={faExclamationTriangle}/> Erro: {err}</div>; }
        ...
        <Fragment>
            ...
            <div id="notifications-layout">
                ...
                {/* Coluna Direita: Tabela */}
                 <div id="notif-table-column">
                     {/* ... (Card, Controles, Loading/Error, Tabela, Paginação) ... */}
                                     ...
                                     <table id="notifications-table-element" className="notifications-table">
                                         ...
                                         <tbody>
                                             ...
                                             {paginatedConfigs.map((group) => {
                                                 ...
                                                 return ( <tr key={group.key} className={group.isActive ? '' : 'inactive-row'}> {/* Usa isActive */}
                                                     ...
                                                     <td data-label="Estado" className="cell-center"><span className={`status ${group.isActive ? 'active' : 'inactive'}`}>{group.isActive ? 'Ativo' : 'Inativo'}</span></td> {/* Usa isActive */}
                                                     <td data-label="Ações" className="action-buttons">
                                                         ...
                                                         <button onClick={() => singleOriginalId && handleTableAction('toggle', singleOriginalId, group.isActive)} title={isGrouped ? "Ação desabilitada" : (group.isActive ? 'Desativar' : 'Ativar')} disabled={isGrouped || actionLoading || isSubmitting} className={`action-button toggle ${group.isActive ? 'toggle-off' : 'toggle-on'}`}><FontAwesomeIcon icon={group.isActive ? faToggleOff : faToggleOn} /></button> {/* Usa isActive */}
